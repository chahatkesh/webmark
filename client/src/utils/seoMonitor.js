...
async function analyzePage(url, _options = {}) {
  ...
  return {
    ...
    checks: {
      title: {
        pass: true,
        value: "Webmark - Modern Bookmark Management Application",
      },
      metaDescription: {
        pass: true,
        value: "Simplify your digital life with Webmark...",
      },
...
async function generateFullSEOReport() {
  ...
  const report = {
    ...
    sitewide: {
      sitemap: {
        exists: true,
        valid: true,
        url: "https://webmark.chahatkesh.me/sitemap-index.xml",
      },
      robots: {
        exists: true,
        valid: true,
        url: "https://webmark.chahatkesh.me/robots.txt",
      },
  ...
  try {
    ...
    const reportFile = path.join(
      REPORTS_DIR,
      `seo-report-${timestamp.split("T")[0]}.json`,
    );
...
async function compareSEOReports(previousDate) {
  try {
    ...
    for (const [pagePath, currentChecks] of Object.entries(
      currentReport.pages,
    )) {
        ...
        for (const [checkName, currentValue] of Object.entries(currentChecks)) {
          ...
          if (JSON.stringify(currentValue) !== JSON.stringify(previousValue)) {
            pageChanges[checkName] = {
              ...
              improved:
                currentValue.pass && (!previousValue || !previousValue.pass),
            };
            hasChanges = true;
          }
        }
...
export { analyzePage, generateFullSEOReport, compareSEOReports };
