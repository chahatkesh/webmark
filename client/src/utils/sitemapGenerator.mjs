...
export function writeSitemap() {
  ...
  try {
    ...
    const sitemapFeaturesPath = path.join(
      __dirname,
    ...
    if (fs.existsSync(sitemapFeaturesPath)) {
      ...
      console.log(
        `Features sitemap updated with current date: ${CURRENT_DATE}`,
      );
    }
    ...
    if (fs.existsSync(robotsPath)) {
      ...
      if (!robotsContent.includes("User-agent: Googlebot")) {
        ...
        if (robotsContent.includes("User-agent: *")) {
          ...
          const generalRulesEnd =
            robotsContent.indexOf("User-agent: *") +
            robotsContent
              .substring(robotsContent.indexOf("User-agent: *"))
              .indexOf("\n\n");
...
`;
      ...
      console.log(
        "Updated robots.txt with all sitemap references and bot-specific rules",
      );
    }
  } catch (error) {
